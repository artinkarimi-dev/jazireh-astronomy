...
export default function NewsPreview({ items = [], content = {} }) {
  return (
    <article className="surface-card home-feature-card accent-orange p-5 sm:p-6">
      ...
      <div className="mt-5 flex-1 space-y-3">
        {items.length ? items.slice(0, 3).map((item, index) => (
          ...
          <div className="rounded-2xl border border-white/[.06] bg-white/[.018] p-4 text-sm leading-7 text-slate-500">
            هنوز خبر معتبری از WordPress برای نمایش دریافت نشده است.
          </div>
        )}
      </div>
