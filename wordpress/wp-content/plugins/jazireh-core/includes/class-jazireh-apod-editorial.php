...
final class Jazireh_APOD_Editorial
{
    ...
    private static function sanitize_source_item(array $item)
    {
        return array(
            ...
            'sourceName' => sanitize_text_field((string) ($item['sourceName'] ?? 'NASA Astronomy Picture of the Day')),
            'copyright' => sanitize_text_field((string) ($item['copyright'] ?? '')),
            'credit' => sanitize_text_field((string) ($item['credit'] ?? ($item['copyright'] ?? ''))),
            'photographer' => sanitize_text_field((string) ($item['photographer'] ?? ($item['copyright'] ?? ''))),
            'sourceUrl' => esc_url_raw((string) ($item['sourceUrl'] ?? '')),
            'mediaUrl' => esc_url_raw((string) ($item['mediaUrl'] ?? '')),
    ...
    public static function source_hash(array $item)
    {
        $source = array(
            ...
            'mediaType' => sanitize_key((string) ($item['mediaType'] ?? 'image')),
            'sourceName' => self::normalize_for_hash($item['sourceName'] ?? ''),
            'copyright' => self::normalize_for_hash($item['copyright'] ?? ($item['photographer'] ?? '')),
            'credit' => self::normalize_for_hash($item['credit'] ?? ($item['copyright'] ?? ($item['photographer'] ?? ''))),
            'sourceUrl' => esc_url_raw((string) ($item['sourceUrl'] ?? '')),
            'mediaUrl' => esc_url_raw((string) ($item['mediaUrl'] ?? '')),
            'hdUrl' => esc_url_raw((string) ($item['hdUrl'] ?? '')),
            'serviceVersion' => sanitize_text_field((string) ($item['serviceVersion'] ?? '')),
